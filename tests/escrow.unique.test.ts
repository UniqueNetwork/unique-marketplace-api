...
describe('Escrow test', () => {
  ...
  const deployContract = async (config, admin: IKeyringPair) => {
    ...
    const contractAbi = new web3.eth.Contract(JSON.parse(readBCStatic('MarketPlace.abi')), undefined, {
      from: contractOwner.address, ...lib.GAS_ARGS,
    });
  ...
  it('With escrow', async () => {
    ...
    const config = app.get('CONFIG');
    let blocks = {
      start: (await api.rpc.chain.getHeader()).number.toNumber(),
      latest: 0,
      async updateLatest() {
    ...
    for (let tokenId of [sellTokenId, cancelTokenId]) {
      ...
      await expect(util.normalizeAccountId(await explorer.getTokenOwner(collectionId, tokenId))).toEqual({Ethereum: lib.subToEthLowercase(seller.address)})
    }
    ...
    const addAsk = async (tokenId) => {
      ...
      // Add ask to contract
      res = await lib.executeEthTxOnSub(web3, api, seller, contract, m => m.addAsk(PRICE, '0x0000000000000000000000000000000000000001', evmCollection.options.address, tokenId));
      await expect(res.success).toBe(true);

      // Token is transferred to matcher
      await expect(util.normalizeAccountId(await explorer.getTokenOwner(collectionId, tokenId))).toEqual({Ethereum: contract.options.address.toLowerCase()});
    ...
    // Cancelled ask
    {
      ...
      await expect(util.normalizeAccountId(await explorer.getTokenOwner(collectionId, cancelTokenId))).toEqual(util.normalizeAccountId({Ethereum: lib.subToEth(seller.address)}));
    }
    ...
    // Buy
    {
      ...
      await lib.executeEthTxOnSub(web3, api, buyer, contract, m => m.buyKSM(evmCollection.options.address, sellTokenId, lib.subToEth(buyer.address), lib.subToEth(buyer.address)));
    ...
    await expect(util.normalizeAccountId(await explorer.getTokenOwner(collectionId, sellTokenId))).toEqual(util.normalizeAccountId({Ethereum: lib.subToEthLowercase(buyer.address)}));

    // Transfer token to substrate side of buyer
    await unique.signTransaction(
      buyer,
      api.tx.unique.transferFrom(util.normalizeAccountId({Ethereum: lib.subToEth(buyer.address)}), util.normalizeAccountId({Substrate: buyer.address}), collectionId, sellTokenId, 1),
      'api.tx.unique.transferFrom'
    );

    // Token is transferred to substrate account of buyer, seller received funds
    await expect(util.normalizeAccountId(await explorer.getTokenOwner(collectionId, sellTokenId))).toEqual(util.normalizeAccountId({Substrate: buyer.address}));
  });
});