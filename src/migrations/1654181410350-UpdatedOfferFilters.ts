...
export class UpdatedOfferFilters1654181410350 implements MigrationInterface {
    name = 'UpdatedOfferFilters1654181410350'
    public async up(queryRunner: QueryRunner): Promise<void> {
      await queryRunner.query(`create view v_offires_filters as ` +
        `select "offer"."id"                  AS "offer_id", ` +
        `"offer"."status"              AS "offer_status", ` +
        `"offer"."collection_id"       AS "offer_collection_id", ` +
        `"offer"."token_id"            AS "offer_token_id", ` +
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
      await queryRunner.query(`drop view v_offires_filters`);
    }
