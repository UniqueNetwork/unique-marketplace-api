...
export class UpdateSearchIndex1653917799102 implements MigrationInterface {
  name = 'UpdateSearchIndex1653917799102';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    for (const item of await queryRunner.query(
      ...
      const total = await this.getTotalItem(
        queryRunner,
        item.collection_id,
        item.token_id,
      );
