...
export class EscrowService {
    ...
    async getBlockCreatedAt(blockNum: bigint | number, network?: string, blockTimeSec: bigint = 6n): Promise<Date> {
        ...
        block = await repository
            ...
            .where('blockchain_block.network = :network AND blockchain_block.block_number < :num', { network: this.getNetwork(network), num: blockNum })
            .limit(1)
            .getOne();
        if (!!block) {
    ...
    async cancelAsk(collectionId: number, tokenId: number, blockNumber: bigint, network?: string) {
        ...
        await repository.update(
            { collection_id: collectionId.toString(), token_id: tokenId.toString(), status: ASK_STATUS.ACTIVE, network: this.getNetwork(network) },
